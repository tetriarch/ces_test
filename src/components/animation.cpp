...
void AnimationComponent::attach() {
    for(auto& [name, file] : animationFiles_) {
        ...
        if(animation) {
            animations_.insert(
                std::pair<std::string, std::shared_ptr<AnimationData>>(name, animation));
        }
    }
...
void AnimationComponent::addAnimationFiles(
    const std::unordered_map<std::string, std::string>& animationFiles) {
    if(animationFiles.empty()) {
        ERROR("[ANIMATION COMPONENT]: " + entity()->name() +
              " failed to add animation files, the list is empty");
        return;
    }
...
void AnimationComponent::playAnimation(const std::string& name) {
    if(auto it = animations_.find(name); it != animations_.end()) {
        ...
        ERROR(
            "[ANIMATION COMPONENT]: failed to play animation " + name + " for " + entity()->name());
    }
}

void AnimationComponent::playAnimation(
    const std::string& name, std::function<void()> onAnimationEnd) {
    if(auto it = animations_.find(name); it != animations_.end()) {
        reset();
        currentAnimation_ = it->second;
        index_ = it->second->index;
...
void AnimationComponent::update(const f32 dt) {
    ...
    if(animation->frameDuration <= 0.0f) {
        ERROR_ONCE(
            "[ANIMATION COMPONENT]: invalid frame duration in animation: " + animation->name);
    }
